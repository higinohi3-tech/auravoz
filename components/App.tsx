  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] w-[90%] max-w-lg">
        <div className="bg-slate-900 text-white p-4 rounded-2xl shadow-2xl border border-violet-500/30 flex items-center justify-between gap-4 backdrop-blur-lg bg-opacity-95">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-violet-600 rounded-full flex items-center justify-center animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-left mb-12">
            <div>
              ...
              <p className="text-gray-400 max-w-sm mx-auto md:mx-0">
                Líder em tecnologia de voz generativa. A AuraVoz traz emoção e identidade para cada palavra falada.
              </p>
            </div>
            
            <div className="flex flex-col items-center md:items-start">
              <h4 className="text-lg font-bold mb-6 text-violet-400">Recebimento</h4>
              <p className="text-gray-400 text-sm mb-4">Todo pagamento via:</p>
              <div className="bg-gray-800 border-2 border-violet-500/30 rounded-2xl p-5 w-full max-w-xs shadow-lg shadow-violet-500/5">
                <p className="text-[10px] text-violet-400 uppercase font-bold tracking-widest mb-1">IBAN / Número de Conta</p>
                <p className="text-xl font-mono text-white tracking-wider select-all">{accountNum}</p>
                <div className="mt-2 flex gap-2">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                  <span className="text-[9px] text-slate-500 font-bold uppercase">Pagamento Instantâneo</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center md:items-end">
              ...
              <div className="flex space-x-6">
                ...
                <a href={`https://wa.me/${whatsappNum}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-emerald-500 transition-colors text-2xl">
                  <i className="fa-brands fa-whatsapp"></i>
                </a>
              </div>
              <p className="text-gray-500 text-xs mt-6 italic">WhatsApp: +244 931 079 190</p>
            </div>
          </div>
